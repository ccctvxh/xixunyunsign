...
```bash
./xixunyunsign.exe login -a 22070230111 -p yourpassword
```
...
```bash
./xixunyunsign.exe query -a 22070230111
```
...
```bash
./xixunyunsign.exe sign -a 22070230111 --address "江苏省常州市武进区前村一路45号靠近贺北村第三工业园"
```
...
```bash
./xixunyunsign.exe sign -a 22070230111 \
...
本项目采用 MIT 许可证进行许可。有关更多信息，请参阅 [LICENSE](LICENSE) 文件。
